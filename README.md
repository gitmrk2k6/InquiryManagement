...
> なお、本番環境は学習終了後に `terraform destroy` で削除済みのため、現在は上記 URL へのアクセスはできない。
...
> 本番環境は学習終了後に `terraform destroy` で削除済み。
...
**本番アクセス URL**: <http://54.248.22.131>/ （学習終了後に `terraform destroy` で削除済み）
